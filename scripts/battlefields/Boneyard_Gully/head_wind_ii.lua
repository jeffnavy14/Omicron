...
local content = Battlefield:new({
    ...
    battlefieldId         = xi.battlefield.id.HEAD_WIND_II,
    canLoseExp            = false,
    allowTrusts           = true,
    maxPlayers            = 6,
    levelCap              = 99,
    ...
    requiredKeyItems = { xi.ki.HEAD_WIND_PHANTOM_GEM, keep = false }, 
...
local msgOffsets =
{
    [ID.mob.SHIKAREE_X_HW_HTBF] = ID.text.SHIKAREE_X_OFFSET,
    [ID.mob.SHIKAREE_Y_HW_HTBF] = ID.text.SHIKAREE_Y_OFFSET,
    [ID.mob.SHIKAREE_Z_HW_HTBF] = ID.text.SHIKAREE_Z_OFFSET,
}
...
local skillchainData =
{
    ...
    [ID.mob.SHIKAREE_X_HW_HTBF] =
    {
        -- All shikaree are alive: X Evisceration → Y Vorpal Scythe → Z Impulse Drive
        [allAlive] =
        {
            { mobId = ID.mob.SHIKAREE_X_HW_HTBF, skill = xi.mobSkill.EVISCERATION  },
            { mobId = ID.mob.SHIKAREE_Y_HW_HTBF, skill = xi.mobSkill.VORPAL_SCYTHE },
            { mobId = ID.mob.SHIKAREE_Z_HW_HTBF, skill = xi.mobSkill.IMPULSE_DRIVE },
        },
        -- Shikaree X and Y are alive: X Evisceration → Y Vorpal Scythe
        [ID.mob.SHIKAREE_Y_HW_HTBF] =
        {
            { mobId = ID.mob.SHIKAREE_X_HW_HTBF, skill = xi.mobSkill.EVISCERATION  },
            { mobId = ID.mob.SHIKAREE_Y_HW_HTBF, skill = xi.mobSkill.VORPAL_SCYTHE },
        },
        -- Shikaree X and Z are alive: X Dancing Edge → Z Penta Thrust
        [ID.mob.SHIKAREE_Z_HW_HTBF] =
        {
            { mobId = ID.mob.SHIKAREE_X_HW_HTBF, skill = xi.mobSkill.DANCING_EDGE },
            { mobId = ID.mob.SHIKAREE_Z_HW_HTBF, skill = xi.mobSkill.PENTA_THRUST },
        },
    },
    ...
    [ID.mob.SHIKAREE_Y_HW_HTBF] =
    {
        ...
        [allAlive] =
        {
            { mobId = ID.mob.SHIKAREE_Y_HW_HTBF, skill = xi.mobSkill.GUILLOTINE_1    },
            { mobId = ID.mob.SHIKAREE_X_HW_HTBF, skill = xi.mobSkill.SHADOWSTITCH    },
            { mobId = ID.mob.SHIKAREE_Z_HW_HTBF, skill = xi.mobSkill.WHEELING_THRUST },
        },
        -- Shikaree Y and X are alive: Y Spiral Hell → X Shadowstitch
        [ID.mob.SHIKAREE_X_HW_HTBF] =
        {
            { mobId = ID.mob.SHIKAREE_Y_HW_HTBF, skill = xi.mobSkill.SPIRAL_HELL  },
            { mobId = ID.mob.SHIKAREE_X_HW_HTBF, skill = xi.mobSkill.SHADOWSTITCH },
        },
    ...
    [ID.mob.SHIKAREE_Z_HW_HTBF] =
    {
        ...
        [allAlive] =
        {
            { mobId = ID.mob.SHIKAREE_Z_HW_HTBF, skill = xi.mobSkill.SKEWER       },
            { mobId = ID.mob.SHIKAREE_Y_HW_HTBF, skill = xi.mobSkill.SPIRAL_HELL  },
            { mobId = ID.mob.SHIKAREE_X_HW_HTBF, skill = xi.mobSkill.EVISCERATION },
        },
        ...
        {
            { mobId = ID.mob.SHIKAREE_Z_HW_HTBF, skill = xi.mobSkill.WHEELING_THRUST },
            { mobId = ID.mob.SHIKAREE_X_HW_HTBF, skill = xi.mobSkill.SHARK_BITE      },
        },
...
local function checkForNewLeader(battlefield, offsetID)
    for i = 0, 2 do
        local currentID   = offsetID + i
        local baseID      = ID.mob.SHIKAREE_Z_HW_HTBF + i
        local shikareeMob = GetMobByID(currentID)
...
function content:onBattlefieldTick(battlefield, tick)
    ...
    local offset     = (battlefield:getArea() - 1) * 5
    local offsetID   = ID.mob.SHIKAREE_Z_HW_HTBF + offset
...
{
    {
        mobIds =
        {
            ...
            {
                ID.mob.SHIKAREE_Z_HW_HTBF + 5,
                ID.mob.SHIKAREE_Y_HW_HTBF + 5,
                ID.mob.SHIKAREE_X_HW_HTBF + 5,
            },

            {
                ID.mob.SHIKAREE_Z_HW_HTBF + 10,
                ID.mob.SHIKAREE_Y_HW_HTBF + 10,
                ID.mob.SHIKAREE_X_HW_HTBF + 10,
            },
        },
...

    {
        mobIds =
        {
            { ID.mob.SHIKAREE_Z_HW_HTBF +  4 },
            { ID.mob.SHIKAREE_Z_HW_HTBF +  9 },
            { ID.mob.SHIKAREE_Z_HW_HTBF + 14 },
        },
    ...
    -- NOTE: Rabbit is spawned on start, but wyvern is a result of an on engage action.
    {
        mobIds =
        {
            { ID.mob.SHIKAREE_Z_HW_HTBF +  3 },
            { ID.mob.SHIKAREE_Z_HW_HTBF +  8 },
            { ID.mob.SHIKAREE_Z_HW_HTBF + 13 },
        },
...
{
    {
        { itemId = xi.item.COPY_OF_REMS_TALE_CHAPTER_8,         weight =  333}, 
        { itemId = xi.item.COPY_OF_REMS_TALE_CHAPTER_9,         weight =  333}, 
        { itemId = xi.item.COPY_OF_REMS_TALE_CHAPTER_10,        weight =  333}, 
    },
    {
        { itemId = xi.item.NONE,                                weight = 750 }, -- nothing
        { itemId = xi.item.COPY_OF_REMS_TALE_CHAPTER_8,         weight =  82}, 
        { itemId = xi.item.COPY_OF_REMS_TALE_CHAPTER_9,         weight =  82}, 
        { itemId = xi.item.COPY_OF_REMS_TALE_CHAPTER_10,        weight =  82}, 

    },
    {
        { itemId = xi.item.NONE,                            weight = 250 }, -- nothing
        { itemId = xi.item.PIECE_OF_MALIYAKALEYA_CORAL,     weight = 187 }, 
        { itemId = xi.item.HEPATIZON_ORE,                   weight = 187 },
        { itemId = xi.item.EXALTED_LOG,                     weight = 187 }, 
        { itemId = xi.item.CHUNK_OF_BERYLLIUM_ORE,          weight = 187 }, 
        { itemId = xi.item.WYRM_BLOOD,                      weight = 187 }, 
    },
    {
        { itemId = xi.item.NONE,                    weight = 500 }, -- nothing
        { itemId = xi.item.NILGAL_POLE,             weight = 10 }, -- 
        { itemId = xi.item.CHIDORI,                 weight = 10 }, -- 
    },
    {
        { itemId = xi.item.NONE,                    weight = 50 }, -- nothing
        { itemId = xi.item.SHETAL_STONE,            weight = 10 }, -- 
        { itemId = xi.item.BAGHERE_SALADE,          weight = 2 }, 
        { itemId = xi.item.DURGAI_LEGGINGS,         weight = 10 },
    },
}
