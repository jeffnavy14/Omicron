...
xi.events.domainCampaign_shards = xi.events.domainCampaign_shards or {}

xi.events.domainCampaign_shards.onTrigger = function(player, csid)

    local escha = player:getCharVar("[Domain]Shards")
...
xi.events.domainCampaign_shards.onEventUpdate = function(player, csid, option)
    ...
    then
        ...
        if npcUtil.giveItem(player, { {domainInvasion[showItems - 2]["items"][itemSelected + 1], itemQuantity} }) then
            player:incrementCharVar("[Domain]Shards", domainInvasion[showItems - 2]["price"] * itemQuantity)
            player:updateEvent(
                domainInvasion[showItems - 2]["items"][itemSelected + 1],
                player:getCharVar("[Domain]Shards"), -- Login Points after purchase
