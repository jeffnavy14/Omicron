...
entity.onTrade = function(player, npc, trade)
    ...
    local byakk = npcUtil.tradeHas(trade, {{ 3278, 5 }})
	    ...
	    if npcUtil.tradeHas(trade, {{3277, 3}}, 1418, 1419) then
	        player:tradeComplete()
        	npcUtil.giveKeyItem(player, xi.keyItem.SEIRYUS_NOBILITY)
        end
        if npcUtil.tradeHas(trade, {{3276, 3}}, 1420, 1421) then
	        player:tradeComplete()
        	npcUtil.giveKeyItem(player, xi.keyItem.SUZAKUS_BENEFACTION)
        end
        if npcUtil.tradeHas(trade, {{3278, 3}}, 1422, 1423) then
	        player:tradeComplete()
        	npcUtil.giveKeyItem(player, xi.keyItem.BYAKKOS_PRIDE)
        end
        if npcUtil.tradeHas(trade, {{3275, 3}}, 1424, 1425) then
	        player:tradeComplete()
        	npcUtil.giveKeyItem(player, xi.keyItem.GENBUS_HONOR)
        end       
        if npcUtil.tradeHas(trade, 1404, 1405, 1406, 1407) and genbu and suzac and seriu and byakk  then
	        player:tradeComplete()
...
entity.onTrigger = function(player, npc)
    if not player:hasKeyItem(xi.ki.CERULEAN_CRYSTAL) and
