...
xi.custom_htbf.configs[8] = {
            enabled = true,
            name = blackStar .. "Summoner's Vengeance",
            meritCost = 1,
            mobName = { "Carbuncle Prime", "Diabolos Prime", "Fenrir Prime", "Garuda Prime", "Ifrit Prime", "Leviathan Prime", "Ramuh Prime", "Shiva Prime", "Titan Prime" },
            spawnDynamic = function(zone)
                package.loaded["scripts/zones/Batallia_Downs/mobs/Summoners_Vengeance_HTBF"] = nil
                local svScript = require("scripts/zones/Batallia_Downs/mobs/Summoners_Vengeance_HTBF")
                if svScript and svScript.spawn then
                    svScript.spawn(zone)
                end
            end,
            teleport = { x = 186.7012, y = 11.7963, z = -543.1295, rot = 53, zoneId = xi.zone.BATALLIA_DOWNS },
            clearRadius = 120,
            timeLimit = 30,
            remChapters = {
            ...
            difficulties = {
                { name = "Very Easy (Lvl: 113)",      mobLevel = 119, hppMod = 0, dropRateBonus = 0, statModMultiplier = 0.5, personalLoot = { remCount = 1, remItemCount = 0 } },
                { name = "Easy (Lvl: 116)",           mobLevel = 124, hppMod = 25, dropRateBonus = 10, statModMultiplier = 0.6, personalLoot = { remCount = 1, remItemCount = 1 } },
                { name = "Normal (Lvl: 119)",         mobLevel = 129, hppMod = 75, dropRateBonus = 20, statModMultiplier = 0.7, personalLoot = { remCount = 2, remItemCount = 1 } },
                { name = "Difficult (Lvl: 119+)",     mobLevel = 134, hppMod = 100, dropRateBonus = 40, statModMultiplier = 0.85, personalLoot = { remCount = 3, remItemCount = 2 } },
                { name = "Very Difficult (Lvl: ???)", mobLevel = 140, hppMod = 200, dropRateBonus = 90, statModMultiplier = 1.0, personalLoot = { remCount = 4, remItemCount = 2 } },
            },
            lootDelay = 10,
            loot = {
    ...
    player:timer(180000, function(p_timed)
        ...
        if destZone then
            ...
            if not stillActive then
                SetServerVariable("Custom_HTBF_Active_Config_" .. lookup.configIndex, 0)
...
local function checkConfrontation(lookupKey)
    ...
    for _, m in ipairs(mobs) do
        if m:isAlive() then validMobCount = validMobCount + 1 end
    end
...
local function enterBattlefield(player, configId, difficultyIndex, npc)
    ...
    local lockVar = "Custom_HTBF_Active_Config_" .. configId
    
    if GetServerVariable(lockVar) == 1 then
...
entity.onTrigger = function(player, npc)
    ...
    local confluxConfigs = {8}
    local isOccupied = false

    for _, myConfigId in ipairs(confluxConfigs) do
        local config = xi.custom_htbf.configs[myConfigId]
        local lockVar = "Custom_HTBF_Active_Config_" .. myConfigId
        if config and config.enabled then
            if GetServerVariable(lockVar) == 1 then
                local confID = myConfigId + 1000
                local lookup = xi.confrontation.lookup[confID]
                local isStale = false
                
                if not lookup then
                    isStale = true
                elseif os.time() > (lookup.timeLimit + 15) then
                    isStale = true
                else
                    local elapsed = os.time() - (lookup.startTime or (lookup.timeLimit - (config.timeLimit or 30) * 60))
                    if elapsed > 45 then
                        local destZone = GetZone(config.teleport.zoneId)
                        if destZone then
                            local activePlayers = 0
                            for _, pid in ipairs(lookup.registeredPlayerIds) do
                                local p = GetPlayerByID(pid)
                                if p and p:getZoneID() == config.teleport.zoneId then
                ...
                if isStale then
                    if lookup and lookup.cleanUp then lookup.cleanUp(true) end
                    SetServerVariable(lockVar, 0)
                    xi.confrontation.lookup[confID] = nil
                    local destZone = GetZone(config.teleport.zoneId)
                    if destZone then
                        ...
                        respawnHiddenMobs(config.teleport.zoneId)
                    end
                else
                    isOccupied = true
                end
