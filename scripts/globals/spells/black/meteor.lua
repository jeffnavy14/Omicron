...
spell_object.onSpellCast = function(caster, target, spell)
    ...
    --Spell Base Damage = MAB/MDB*floor(Int + Elemental Magic Skill/5)*3.5
    ...
    local dmg = 0
    if (caster:isPC()) then
        dmg = ((100+caster:getMod(xi.mod.MATT))/(100+target:getMod(xi.mod.MDEF))) * (caster:getStat(xi.mod.INT) + caster:getSkillLevel(xi.skill.ELEMENTAL_MAGIC)/5) * 3.5
    else
        dmg = ((100+caster:getMod(xi.mod.MATT))/(100+target:getMod(xi.mod.MDEF))) * (caster:getStat(xi.mod.INT) + (caster:getMaxSkillLevel(caster:getMainLvl(), xi.job.BLM, xi.skill.ELEMENTAL_MAGIC))/5) * 9.4
    end
